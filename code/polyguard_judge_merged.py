"""Run PolyGuard over prompt/response pairs in a JSON array.

Example:
    python polyguard_judge_merged.py input.json output.json
"""
...
RAW_FIELD = "polyguard_raw_output"
...
def render_prompt(tokenizer: Any, prompt: str, response: str) -> str:
    messages = [
        ...
        {
            "role": "user",
            "content": POLYGUARD_USER_TEMPLATE.format(prompt=prompt, response=response),
        },
    ]
